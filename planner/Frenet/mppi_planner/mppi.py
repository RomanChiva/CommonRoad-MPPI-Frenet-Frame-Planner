...
class MPPIPlanner(ABC):
    ...
    def get_trajectories(self, state, **kwargs):
        """
    ...
    def evaluate_trajectories(self, drop, predictions, pred_cov, plan_cov, n_samples, discount, weight):
        ...
        mask = torch.ones(self.K, dtype=torch.bool, device=self.tensor_args['device'])
        mask[drop] = False
    ...
    def command(self, state):
        ...
        if self.mppi_mode == 'simple':
           ...
           # Compute how many values above 0.0001
            ...
            top_values, top_idx = torch.topk(self.omega, 100)
            #print(top_values, 'Highest Weighted Samples')
    ...
    def propagate_traj(self, perturbed_actions):
        """
    ...
    def perturb_and_prop_state(self):
        """
        ...
        actions, trajs = self.propagate_traj(self.perturbed_action)

        return actions, trajs


    def compute_costs_filtered(self):




        K, T, nu = self.actions.shape 


        cost_total = torch.zeros(K, device=self.tensor_args['device'], dtype=self.tensor_args['dtype'])
        cost_samples = cost_total

        c = self._running_cost(self.states)
