...
from mppi_utils.mppi_utils import generate_gaussian_halton_samples, scale_ctrl, cost_to_go
import time
...
class MPPIPlanner(ABC):
    ...
    def command(self, state, obst):
        ...
        if self.mppi_mode == 'simple':
            ...
            cost_total, states = self._compute_total_cost_batch_halton(obst)
    ...
    def _compute_rollout_costs(self, perturbed_actions, obst):
        """
        ...
        vel_horizon = torch.zeros([K, T], device=self.tensor_args['device'], dtype=self.tensor_args['dtype'])
        ...
        print("cost determination computational time is: ", end_time - start_time)
    ...
    def _compute_total_cost_batch_halton(self, obst):
        """
        ...
        self.cost_total, self.states, self.actions = self._compute_rollout_costs(self.perturbed_action, obst)
        ...
        # self.cost_total += perturbation_cost
        return self.cost_total, self.states
