...
class ScenarioHandler:
    ...
    def _initialize(self):
        ...
        print('PLANNING PROBLEM SET:', self.planning_problem_set.__dict__)
        ...
        print('AGENTS THAT DO THE PLANNING:', self.agent_planning_problem_id_assignment)
        print('ALL DYNAMIC OBSTACLES IN SCENARIO:', [obst.obstacle_id for obst in self.scenario.dynamic_obstacles])
        ...
        for dynamic_obstacle in self.scenario.dynamic_obstacles:
            ...
            if (dynamic_obstacle.obstacle_id in self.agent_planning_problem_id_assignment):
                # Create a planning agent for the 'active' vehicles
                self._create_planner_agent_for_ego_vehicle(dynamic_obstacle.obstacle_id)

        self.collision_checkers = {}
        # CReate a collision checker for each agent to control collisions with scenario and other agents?
        with self.exec_timer.time_with_cm("initialization/initialize collision checker"):

            for agent in self.agent_list:
                # Print agent ID
                cc_scenario_agent = copy.deepcopy(self.scenario)
                cc_scenario_agent.remove_obstacle(
                    obstacle=[cc_scenario_agent.obstacle_by_id(agent.agent_id)])
                try:
                    self.collision_checkers[agent.agent_id] = create_collision_checker(cc_scenario_agent)
                except Exception:
    ...
    def _check_collision(self, agent, time_step, collision_checker):
        ...
        if not collision_checker.collide(current_state_collision_object):
            return
...
class ScenarioHandlerOld:
    ...
    def _initialize(self):
        ...
        # create a collision checker (remove the ego vehicle from the scenario)
